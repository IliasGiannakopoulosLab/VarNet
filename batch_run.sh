...
VN_FILE_SPLIT=${VN_FILE_SPLIT:-even}
...
RUN_UNCERTAINTY_CALIBRATION=${RUN_UNCERTAINTY_CALIBRATION:-true}
RUN_UNCERTAINTY_TESTING=${RUN_UNCERTAINTY_TESTING:-true}
...
if [ "${RUN_TRAINING}" = "true" ]; then
    srun python3 "${VARNET_DIR}/runner.py" "${COMMON_ARGS[@]}" "${MASK_ARGS[@]}" "${VN_SPLIT_ARGS[@]}" "${TRAIN_MODE_ARGS[@]}"
fi

VN_CHECKPOINT_PATH=$(resolve_preferred_ckpt "${CHECKPOINT_DIR}")

if [ "${RUN_TESTING}" = "true" ]; then
    srun python3 "${VARNET_DIR}/runner.py" "${COMMON_ARGS[@]}" "${MASK_ARGS[@]}" --mode "${TEST_MODE}" --test_path "${VN_TEST_PATH}"
fi
