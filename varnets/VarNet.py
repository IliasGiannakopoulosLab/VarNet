...
from data.learnable_mask import LearnableCartesianMask, Learnable2DCartesianMask
from utilities.functions import ifft2c_new as ifft2c
from utilities.functions import (
    batched_mask_center,
    batched_mask_center_3d,
    rss_complex,
    rss,
    complex_abs,
    image_crop,
...
class UNet3D(nn.Module):
    ...
    def __init__(
        ...
        self.up_conv.append(
            nn.Sequential(
                ...
                nn.Conv3d(ch // 2, out_chans, kernel_size=1, stride=1),
            )
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for tconv, conv in zip(self.up_transpose, self.up_conv):
            ...
            # pad if needed
            diffZ = skip.shape[-3] - out.shape[-3]
            diffY = skip.shape[-2] - out.shape[-2]
            diffX = skip.shape[-1] - out.shape[-1]
...
class NormUNet3D(nn.Module):
    ...
    def norm(self, x: Tensor):
        ...
        mean = x_.mean(dim=2).view(b, c, 1, 1, 1)
        std = x_.std(dim=2).view(b, c, 1, 1, 1)
    ...
    # -------------------------------------------#
    # ----------- next multiple helper --------- #
    # -------------------------------------------#
    def _next_multiple(self, value: int, multiple: int) -> int:
        if multiple <= 1:
            return value
    ...
    def pad(self, x: Tensor):
        ...
        mult_d = 2 ** self.num_pools if self.pool_kernel_size[0] > 1 else 1
        mult_h = 2 ** self.num_pools if self.pool_kernel_size[1] > 1 else 1
        mult_w = 2 ** self.num_pools if self.pool_kernel_size[2] > 1 else 1

        d_mult = self._next_multiple(d, mult_d)
        h_mult = self._next_multiple(h, mult_h)
        w_mult = self._next_multiple(w, mult_w)

        pad_d = [math.floor((d_mult - d) / 2), math.ceil((d_mult - d) / 2)]
        pad_h = [math.floor((h_mult - h) / 2), math.ceil((h_mult - h) / 2)]
        pad_w = [math.floor((w_mult - w) / 2), math.ceil((w_mult - w) / 2)]

        x = F.pad(x, pad_w + pad_h + pad_d)

        return x, (pad_d, pad_h, pad_w, d_mult, h_mult, w_mult)

    # -------------------------------------------#
    # ------------------ unpad ----------------- #
    # -------------------------------------------#
    def unpad(self, x: Tensor, pad_d, pad_h, pad_w, d_mult, h_mult, w_mult):
        return x[
            ...,
            pad_d[0] : d_mult - pad_d[1],
            pad_h[0] : h_mult - pad_h[1],
            pad_w[0] : w_mult - pad_w[1],
        ]
...
class ConvBlock3D(nn.Module):
    """
    Memory-friendlier anisotropic 3D convolution block.

    Instead of two full 3x3x3 convolutions, this uses two factorized blocks:

        1 x kH x kW   in-plane convolution
        kD x 1 x 1    through-slice convolution

    For kernel_size=(3,3,3), this becomes:
        1x3x3 + 3x1x1

    This keeps multislice context while reducing the burden of full volumetric
    convolution.
    """
    ...
    def __init__(
        ...
        pad_inplane = (0, kh // 2, kw // 2)
        pad_depth = (kd // 2, 0, 0)

        self.layers = nn.Sequential(
            # First factorized conv
            nn.Conv3d(
                in_chans,
                out_chans,
                kernel_size=(1, kh, kw),
...
# -------------------------------------------#
# ---------- sensitivity model 3d ---------- #
# -------------------------------------------#
class SensitivityModel3D(nn.Module):
    """
    Slice-wise 2D sensitivity model for 3D VarNet.

    Input:
        masked_kspace: [B, C, D, H, W, 2]
        mask:          [B, 1, D, 1, W, 1]

    Output:
        sens_maps:     [B, C, D, H, W, 2]

    This uses the standard 2D sensitivity model slice-by-slice.
    """
    ...
    # -------------------------------------------#
    # -------- load 2d sensitivity weights ------#
    # -------------------------------------------#
    def load_from_2d_checkpoint(
        self,
        ckpt_path: str,
        strict: bool = True,
    ):
        """
        Load only the 2D sensitivity NormUNet weights from a 2D VarNet checkpoint.

        Accepted checkpoint key patterns include:
            fi_varnet.sens_net.norm_unet.*
            fi_varnet.base_varnet.sens_net.norm_unet.*
            varnet.sens_net.norm_unet.*
            base_varnet.sens_net.norm_unet.*
            sens_net.norm_unet.*
            norm_unet.*
        """
        ...
        prefixes = [
            ...
            "fi_varnet.base_varnet.sens_net.norm_unet.",
            "varnet.sens_net.norm_unet.",
            "varnet.base_varnet.sens_net.norm_unet.",
            "base_varnet.sens_net.norm_unet.",
            "sens_net.norm_unet.",
            "norm_unet.",
        ]
        ...
        for prefix in prefixes:
            ...
            if len(filtered) > 0:
                print(
                    f"[SensitivityModel3D] Loading sensitivity weights "
                    f"using prefix: {prefix}"
                )
                break

        # Last fallback: find the substring anywhere in the key.
        if len(filtered) == 0:
            marker = "sens_net.norm_unet."
            filtered = {
            ...
            if len(filtered) > 0:
                print(
                    "[SensitivityModel3D] Loading sensitivity weights "
                    "using substring match: sens_net.norm_unet."
                )

        if len(filtered) == 0:
            raise RuntimeError(
                f"No sensitivity NormUNet weights found in checkpoint: {ckpt_path}"
            )

        missing, unexpected = self.norm_unet.load_state_dict(
            filtered,
            strict=strict,
        )

        print(f"[SensitivityModel3D] Loaded sensitivity checkpoint: {ckpt_path}")
        print(f"[SensitivityModel3D] Missing keys: {len(missing)}")
        print(f"[SensitivityModel3D] Unexpected keys: {len(unexpected)}")

        if len(missing) > 0:
            print("[SensitivityModel3D] First missing keys:", missing[:10])

        if len(unexpected) > 0:
            print("[SensitivityModel3D] First unexpected keys:", unexpected[:10])

    # -------------------------------------------#
    # -------- freeze sensitivity model ---------#
    # -------------------------------------------#
    def freeze(self):
    ...
    def forward(
        ...
        b, c, d, h, w, comp = masked_kspace.shape
        assert comp == 2

        if self.mask_center:

            middle_mask = mask[:, :, d // 2, :, :, :]  # [B, 1, 1, W, 1]
            ...
            masked_kspace = batched_mask_center_3d(
                masked_kspace,
                pad,
                pad + num_low_freqs,
            )
        ...
        for z in range(d):
            ...
            kspace_slice = masked_kspace[:, :, z, :, :, :]

            images, batches = self.chans_to_batch_dim(
                ifft2c(kspace_slice)
            )

            sens_slice = self.norm_unet(images)

            sens_slice = self.batch_chans_to_chan_dim(
                sens_slice,
                batches,
            )

            sens_slices.append(sens_slice.unsqueeze(2))

        sens = torch.cat(sens_slices, dim=2)
...
class E2EVarNet3D(nn.Module):
    ...
    def __init__(
        ...
        pool_kernel_size: tuple = (2, 2, 2),
        kspace_mult_factor: float = 1e6,
        sens_ckpt: Optional[str] = None,
        freeze_sens_net: bool = False,
        strict_sens_load: bool = True,
    ):
        super().__init__()

        self.use_gradient_checkpointing = True
        self.kspace_mult_factor = kspace_mult_factor
        self.freeze_sens_net = freeze_sens_net

        self.sens_net = SensitivityModel3D(
            chans=sens_chans,
            num_pools=sens_pools,
            mask_center=mask_center,
        )

        if sens_ckpt is not None:
            self.sens_net.load_from_2d_checkpoint(
                ckpt_path=sens_ckpt,
                strict=strict_sens_load,
            )

        if self.freeze_sens_net:
            self.sens_net.freeze()
            print("[E2EVarNet3D] Sensitivity network is frozen.")
    ...
    def _compute_sens_maps(
        ...
        if self.freeze_sens_net:
            ...
            return sens_maps.detach()

        if self.training and self.use_gradient_checkpointing:
            return checkpoint(
                lambda masked_kspace_, mask_: self.sens_net(
                    masked_kspace_,
                    mask_,
                    num_low_frequencies,
                ),
    ...
    def forward(
        ...
        sens_maps = self._compute_sens_maps(
            masked_kspace=masked_kspace,
            mask=mask,
            num_low_frequencies=num_low_frequencies,
        )
...
# -------------------------------------------#
# ----- learnable masked varnet 3d --------- #
# -------------------------------------------#
class LearnableMaskedVarNet3D(nn.Module):
    ...
    def __init__(
        ...
        acceleration: int,
        center_fraction: float,
        num_phase_logits: int = 320,
        num_slice_logits: int = 40,
        ...
        self.learnable_mask = Learnable2DCartesianMask(
            acceleration=acceleration,
            center_fraction=center_fraction,
            num_phase_logits=num_phase_logits,
            num_slice_logits=num_slice_logits,
            init_bias=init_bias,
        )
    ...
    def forward(
        ...
        if self.training and self.use_gradient_checkpointing:
            output = checkpoint(
                lambda masked_kspace_, mask_: self.base_varnet(
                    masked_kspace=masked_kspace_,
                    mask=mask_,
                    num_low_frequencies=num_low_frequencies,
                    crop_size=crop_size,
                ),
...
# -------------------------------------------#
# -------- uncertainty network 3d ---------- #
# -------------------------------------------#
class UncertaintyNetwork3D(nn.Module):
    """
    Image-only 3D uncertainty model.

    Input:
        reconstruction volume [B, D, H, W]
    ...
    def __init__(
        ...
        # Hard-coded for the 3D path.
        self.use_gradient_checkpointing = True

        self.lower_head = UNet3D(
            in_chans=1,
            out_chans=1,
            chans=head_chans,
            num_pool_layers=head_pools,
    ...
    def _run_head(
        ...
        if self.training and self.use_gradient_checkpointing:
            return checkpoint(
                lambda recon_in_: _apply_model_with_optional_crop(
                    head,
                    recon_in_,
                    crop_size,
                ),
        ...
        return _apply_model_with_optional_crop(
            head,
            recon_in,
            crop_size,
        )
    ...
    def forward(
        ...
        """
        if recon.ndim != 4:
            raise ValueError(f"Expected recon with shape [B, D, H, W], got {recon.shape}")

        recon_in = recon.unsqueeze(1)  # [B, 1, D, H, W]

        lower_raw = self._run_head(
            self.lower_head,
            recon_in,
            crop_size,
        ).squeeze(1)
        upper_raw = self._run_head(
            self.upper_head,
            recon_in,
            crop_size,
        ).squeeze(1)
...
class VarNetBlock3D(nn.Module):
    ...
    def forward(
        ...
        image = sens_reduce_3d(current_kspace, sens_maps)
        ...
        model_term = sens_expand_3d(model_image, sens_maps)
...
class TinyConv3D(nn.Module):
    """
    Minimal 3D convolutional network for smoke-testing the 3D VarNet wiring.

    Channel path:
        2 -> hidden_chans -> 2

    This is intentionally not a U-Net. It has no pooling, no skip pyramid, and
    therefore much lower activation memory. It is not meant to be the final
    ...
    def __init__(
        ...
        if drop_prob > 0:
            layers.append(nn.Dropout3d(drop_prob))
...
# -------------------------------------------#
# ----------- tiny normalized 3d net --------#
# -------------------------------------------#
class NormTinyConv3D(nn.Module):
    """
    Normalized wrapper around TinyConv3D with the same input/output contract as
    NormUNet3D.

    Accepted input:
        complex image: [B, C, D, H, W, 2]
        or channel image: [B, C, D, H, W] when use_complex=False

    Default VarNet use:
        [B, 1, D, H, W, 2] -> [B, 1, D, H, W, 2]
    """
