...
class UncertaintyModule(pl.LightningModule):
    """
    Separate Lightning module for uncertainty training/testing.

    Design:
      - loads a pretrained VarNet checkpoint (optional, in __init__)
      - freezes VarNet permanently
      - trains only the uncertainty network
      - calibrates lambda here
      - appends only uncertainty_map to pre-existing VarNet H5s during test
      - supports true Lightning resume without storing frozen VarNet weights

    Assumptions:
      - varnet_model returns a reconstruction image [B, H, W]
      - uncertainty_model takes recon [B, H, W] and returns
            lower_pred [B, H, W], upper_pred [B, H, W]
    """
    ...
    def __init__(
        ...
        varnet_ckpt_prefix: str = "fi_varnet.",
        ...
        self.reconstructions_dir = Path(reconstructions_dir) if reconstructions_dir is not None else None
    ...
    def on_save_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        """
        Keep full Lightning trainer state (optimizer/scheduler/loops/etc.) but
        replace the model state_dict so that only uncertainty weights are stored.
        The frozen VarNet is always loaded separately from its own checkpoint.
        """
    ...
    def load_state_dict(self, state_dict: Dict[str, torch.Tensor], strict: bool = True):
        """
        Allow Lightning resume checkpoints that contain only uncertainty_model.*
        weights, while varnet_model is loaded separately from its own checkpoint.
        """
        incompatible = super().load_state_dict(state_dict, strict=False)

        missing = [k for k in incompatible.missing_keys if not k.startswith("varnet_model.")]
        unexpected = list(incompatible.unexpected_keys)

        if strict and (missing or unexpected):
            raise RuntimeError(
                "Error(s) in loading state_dict for UncertaintyModule:\n"
    ...
    def _load_varnet_checkpoint(
        ...
        print(f"[UncertaintyModule] loaded VarNet checkpoint from {ckpt_path}")
        print(f"[UncertaintyModule] missing VarNet keys: {len(missing)}")
        print(f"[UncertaintyModule] unexpected VarNet keys: {len(unexpected)}")
    ...
    def _run_varnet(self, batch) -> torch.Tensor:
        """
        Reuses only the existing input-routing logic of the current VarNet module.
        """
        # Important: Lightning sets the parent module to train() during fitting,
        ...
        if isinstance(recon, tuple):
            raise ValueError(
                "varnet_model returned a tuple. UncertaintyModule expects the VarNet path "
                "to return only the reconstruction image."
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        target, recon, lower_pred, upper_pred = self._crop_to_target(
            batch.target, recon, lower_pred, upper_pred
        )

        loss, lower_loss, upper_loss = self._compute_pinball_loss(
            target, lower_pred, upper_pred
        )
    ...
    def validation_epoch_end(self, val_logs):
        ...
        pinball_lower = torch.mean(torch.cat([x["pinball_lower"].view(-1) for x in val_logs]))
        pinball_upper = torch.mean(torch.cat([x["pinball_upper"].view(-1) for x in val_logs]))
    ...
    def _fraction_missed_loss(
        ...
        """
        Returns one miss fraction per image: [B].

        Do not squeeze here. For B=1, squeeze would remove the batch dimension
        and accidentally return one value per row instead of one value per image.
        """
        ...
        if target.ndim != 3:
            raise ValueError(
                f"Expected 2D target batch with shape [B, H, W], got {target.shape}"
            )
    ...
    def test_step(self, batch, batch_idx):
        if self.calibration_lambda is None:
            raise RuntimeError(
                "calibration_lambda is not set. Run calibrate(...) or load an uncertainty checkpoint first."
            )

        recon, lower_pred, upper_pred = self.forward(batch)

        crop_size = (
            (recon.shape[-1], recon.shape[-1])
            if recon.shape[-1] < batch.crop_size[1]
        ...
        lower_cal, upper_cal = self._apply_lambda(
            recon, lower_pred, upper_pred, self.calibration_lambda
        )
    ...
    def test_epoch_end(self, test_logs):
        if self.reconstructions_dir is None:
            raise ValueError(
                "reconstructions_dir must be set so uncertainty maps can be appended "
                "to the pre-existing VarNet H5 files."
            )

        packed_unc = defaultdict(dict)

        for log in test_logs:
            for i, (fname, slice_num) in enumerate(zip(log["fname"], log["slice"])):
                sl = int(slice_num.cpu())
                packed_unc[fname][sl] = log["uncertainty_map"][i]

        for fname in packed_unc:
            ...
            unc_stack = np.stack(
                [u for _, u in sorted(packed_unc[fname].items())]
            )

            with h5py.File(file_path, "a") as hf:
                if "uncertainty_map" in hf:
                    del hf["uncertainty_map"]
                hf.create_dataset("uncertainty_map", data=unc_stack)

        self.print(
            f"Appended uncertainty maps to existing H5 files in {self.reconstructions_dir}"
        )
    ...
    def load_uncertainty_checkpoint(self, path: Path, strict: bool = True):
        """
        Load either:
          - a full Lightning checkpoint whose state_dict contains uncertainty_model.* keys, or
          - a slim export checkpoint whose state_dict contains only uncertainty_model keys.
        """
    ...
    def update_checkpoint_file(self, src_path: Path, dst_path: Optional[Path] = None):
        """
        Update an existing checkpoint file with the current uncertainty weights and
        calibration lambda. If the source is a full Lightning checkpoint, keep the
        trainer/optimizer/scheduler state and replace only the model state_dict + lambda.
        If the source is a slim export checkpoint, write back a slim export checkpoint.
        """
