...
class Learnable2DCartesianMask(nn.Module):
    """
    MoDL+BS style learnable 2D Cartesian mask for multislice / volume VarNet.

    The mask is 2D over the slice and phase-encoding axes: D x ky.

    What this implements:
      - learn unconstrained canonical 2D mask parameters [num_slice_logits, num_phase_logits]
      - resize them to the current volume shape [D, support_width]
      - keep the same contiguous ky center band fully sampled for every slice
      - renormalize all OUTER probabilities globally over D x ky so the expected
        total number of sampled points matches the requested acceleration
      - allow different slices to receive different outer sampling densities
      - sample a binary 2D outer mask at every forward pass
      - use a straight-through estimator for backprop through the binary sample

    Input shape
    -----------
    full_kspace: [B, coils, D, H, W, 2]
    ...
    def __init__(
        ...
        if not (0.0 < center_fraction < 1.0):
            raise ValueError("center_fraction must be in (0, 1)")
        if num_phase_logits <= 0:
            raise ValueError("num_phase_logits must be positive")
        if num_slice_logits <= 0:
            raise ValueError("num_slice_logits must be positive")

        self.acceleration = acceleration
        self.center_fraction = center_fraction
        self.num_phase_logits = num_phase_logits
        self.num_slice_logits = num_slice_logits

        # Sigmoid slope a used in LOUPE / LOUPE-ST papers.
        self.prob_slope = 0.25

        # Unconstrained canonical parameter map over slice x phase axes.
        if init_bias is None:
            init = torch.empty(num_slice_logits, num_phase_logits).uniform_(-15.0, 15.0)
        else:
            init = torch.full(
                (num_slice_logits, num_phase_logits),
                float(init_bias),
            )
        self.mask_parameters = nn.Parameter(init)

    # -------------------------------------------#
    # -------- resize canonical params 2d ------ #
    # -------------------------------------------#
    def _resize_parameter_map_2d(
        self,
        num_slices: int,
        support_width: int,
    ) -> torch.Tensor:
        ...
        if num_slices == self.num_slice_logits and support_width == self.num_phase_logits:
            return self.mask_parameters

        x = self.mask_parameters.view(
            1,
            1,
            self.num_slice_logits,
            self.num_phase_logits,
        )
        x = F.interpolate(
            x,
            size=(num_slices, support_width),
            mode="bilinear",
            align_corners=True,
        )
        return x.view(num_slices, support_width)

    # -------------------------------------------#
    # ---- fixed center band, repeated in D ----- #
    # -------------------------------------------#
    def _build_center_mask_2d(
        self,
        num_slices: int,
        support_width: int,
        device: torch.device,
        ...
        center_1d = torch.zeros(support_width, device=device, dtype=dtype)
        pad = (support_width - num_low_frequencies + 1) // 2
        center_1d[pad : pad + num_low_frequencies] = 1.0

        center_2d = center_1d.unsqueeze(0).expand(num_slices, support_width).clone()
        return center_2d, num_low_frequencies, pad, pad + num_low_frequencies

    # -------------------------------------------#
    # -------- embed support mask in W --------- #
    # -------------------------------------------#
    def _embed_support_mask_2d(
        self,
        support_mask: torch.Tensor,
        full_width: int,
        acq_start: int,
        ...
        num_slices = support_mask.shape[0]
        full_mask = torch.zeros(num_slices, full_width, device=device, dtype=dtype)
        full_mask[:, acq_start:acq_end] = support_mask
        return full_mask
    ...
    def _renormalize_probabilities(
        ...
        """
        Linear renormalization so mean(probs_out) == target_mean while keeping
        values in [0, 1]. Here `probs` contains all non-center D x ky entries,
        so the constraint is global across slices.
        """
        eps = torch.finfo(probs.dtype).eps
        target_mean = torch.clamp(target_mean, 0.0, 1.0)
    ...
    def forward(
        ...
        if full_kspace.ndim != 6 or full_kspace.shape[-1] != 2:
            raise ValueError("Expected full_kspace with shape [B, coils, D, H, W, 2]")

        device = full_kspace.device
        batch_size = full_kspace.shape[0]
        num_slices = full_kspace.shape[2]
        full_width = full_kspace.shape[-2]
        ...
        num_slices_list = []
        sampled_points = []
        sampled_lines = []  # Compatibility alias: total sampled D x ky points.
        effective_accelerations = []
        per_slice_sampled_lines = []
        per_slice_effective_accelerations = []
        num_low_freqs_list = []
        center_start_list = []
        center_end_list = []
        target_total_ratio_list = []
        target_outer_mean_list = []
        target_sampled_points_list = []
        center_points_list = []
        outer_count_list = []
        ...
        for b in range(batch_size):
            ...
            support_widths.append(float(support_width))
            num_slices_list.append(float(num_slices))

            resized_params = self._resize_parameter_map_2d(
                num_slices=num_slices,
                support_width=support_width,
            )

            center_mask_support, num_low_frequencies, center_start, center_end = self._build_center_mask_2d(
                num_slices=num_slices,
                support_width=support_width,
                device=device,
                dtype=full_kspace.dtype,
            ...
            outer_idx = outer_indicator.bool()
            outer_count = int(outer_idx.sum().item())
            outer_count_list.append(float(outer_count))

            total_support_points = float(num_slices * support_width)
            target_total_ratio = 1.0 / float(self.acceleration)
            target_sampled_points = target_total_ratio * total_support_points
            center_points = float(num_slices * num_low_frequencies)

            target_total_ratio_list.append(target_total_ratio)
            target_sampled_points_list.append(target_sampled_points)
            center_points_list.append(center_points)

            if outer_count > 0:
                target_outer_mean = (target_sampled_points - center_points) / float(outer_count)
                target_outer_mean = float(max(0.0, min(1.0, target_outer_mean)))

                outer_probs_raw = torch.sigmoid(self.prob_slope * resized_params[outer_idx])
                outer_prob_raw_means.append(outer_probs_raw.mean())

                # This is the key difference from independent 1D masks:
                # renormalization is over all outer D x ky entries at once.
                outer_probs = self._renormalize_probabilities(
                    outer_probs_raw,
                    target_mean=outer_probs_raw.new_tensor(target_outer_mean),
                )
            ...
            sampled_points_count = hard_support.sum()
            sampled_points.append(sampled_points_count)
            sampled_lines.append(sampled_points_count)
            effective_accelerations.append(
                resized_params.new_tensor(total_support_points)
                / sampled_points_count.clamp_min(1.0)
            )

            per_slice_counts = hard_support.sum(dim=1)
            per_slice_sampled_lines.append(per_slice_counts)
            per_slice_effective_accelerations.append(
                resized_params.new_tensor(float(support_width))
                / per_slice_counts.clamp_min(1.0)
            )

            prob_full_batch.append(
                self._embed_support_mask_2d(
                    prob_support,
                    full_width=full_width,
                    acq_start=start,
                    acq_end=end,
        ...
        if len(unique_num_low) != 1:
            raise ValueError(
                "MoDL+BS 2D mask produced different center sizes inside the same batch. "
                "Use batch_size=1 or update the VarNet sensitivity path to accept per-sample center sizes."
            )
        num_low_frequencies = unique_num_low[0]

        sampled_hard_full = torch.stack(sampled_hard_full, dim=0)  # [B, D, W]
        sampled_st_full = torch.stack(sampled_st_full, dim=0)      # [B, D, W]
        prob_full_batch = torch.stack(prob_full_batch, dim=0)      # [B, D, W]
        center_full_batch = torch.stack(center_full_batch, dim=0)  # [B, D, W]
        ...
        extras = {
            "prob_mask_2d": prob_full_batch,
            "hard_mask_2d": sampled_hard_full,
            "mask_st_2d": sampled_st_full,
            "center_mask_2d": center_full_batch,
            "support_width": torch.tensor(support_widths, device=device, dtype=torch.float32),
            "num_slices": torch.tensor(num_slices_list, device=device, dtype=torch.float32),
            "sampled_points": torch.stack(sampled_points).to(device=device, dtype=torch.float32),
            "sampled_lines": torch.stack(sampled_lines).to(device=device, dtype=torch.float32),
            "effective_acceleration": torch.stack(effective_accelerations).to(device=device, dtype=torch.float32),
            "per_slice_sampled_lines": torch.stack(per_slice_sampled_lines, dim=0).to(device=device, dtype=torch.float32),
            "per_slice_effective_acceleration": torch.stack(per_slice_effective_accelerations, dim=0).to(device=device, dtype=torch.float32),
            "outer_prob_mean": torch.stack(outer_prob_means).to(device=device, dtype=torch.float32),
            "outer_prob_raw_mean": torch.stack(outer_prob_raw_means).to(device=device, dtype=torch.float32),
            "target_total_ratio": torch.tensor(target_total_ratio_list, device=device, dtype=torch.float32),
            "target_outer_mean": torch.tensor(target_outer_mean_list, device=device, dtype=torch.float32),
            "target_sampled_points": torch.tensor(target_sampled_points_list, device=device, dtype=torch.float32),
            "center_points": torch.tensor(center_points_list, device=device, dtype=torch.float32),
            "outer_count": torch.tensor(outer_count_list, device=device, dtype=torch.float32),
            # Kept for compatibility with the existing module API; not used in the MoDL+BS loss.
            "prob_reg_loss": torch.tensor(0.0, device=device, dtype=torch.float32),
            "num_low_frequencies": num_low_frequencies,
            "num_low_frequencies_per_sample": torch.tensor(num_low_freqs_list, device=device, dtype=torch.float32),
